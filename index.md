# Yasmeen Abuhasirah

**PhD Candidate in Cognitive Computing (Informatique cognitive)**  
Université du Québec à Montréal (UQAM), Canada

---

## About
I am a PhD candidate in Cognitive Computing at the Université du Québec à Montréal (UQAM). My research lies at the intersection of artificial intelligence, cognitive science, and legal analysis, with a focus on building transparent and explainable AI systems for analyzing privacy policies, particularly in the context of children’s mobile games.

My doctoral work combines symbolic approaches (ontologies and rule-based reasoning) with data-driven methods (natural language processing and large language models) to support legally grounded and cognitively informed interpretations of complex regulatory texts.

I am a recipient of the **Fonds de recherche du Québec (FRQ) – Science en exil doctoral scholarship (2025–2028)**.

---

## Research Interests
- Cognitive Computing and Knowledge Representation  
- Legal AI and Explainable AI (XAI)  
- Privacy Policies and Regulatory Compliance  
- Ontologies and Semantic Modeling  
- Natural Language Processing  
- Team Dynamics and AI-generated Dialogue Analysis  

---

## Doctoral Research
**Working title:** Cognitive and Ontology-Driven Analysis of Privacy Policies in Children’s Mobile Games  


**Description:**  
This research aims to develop AI-based methods to analyze the compliance of Terms of Use and Privacy Policies of children’s mobile games with privacy regulations in Québec, Canada, and the United States (e.g., COPPA). The project proposes a two-layer ontology, combining a normative core ontology with a domain-specific privacy ontology, supported by NLP and large language models to enable transparent, explainable, and legally grounded analysis.

---

## Publications
- **Abuhasirah, Y.** (2025). *Simulating Team Dysfunction with ChatGPT*.  
  In Proceedings of the 12th International Conference on Social Networks Analysis, Management and Security (SNAMS 2025).  
  *(Accepted paper, PDF forthcoming)*

- **Abuhasirah, Y.** (2024). *Analyzing and classifying group dynamics dysfunctions in interpersonal dialogues*.  
  Master’s thesis, Université du Québec à Montréal.
- **Abuhasirah, Y.** (2023) *Data Mining Approaches for Depression Detection on Social Media Twitter Dataset*
  In AI-Augmented Digital Twins. Springer, 2024.

---

## Affiliations
- Institut des sciences cognitives (ISC), UQAM — Student Member  
- LATECE Laboratory, UQAM  
- Centre de recherche sur la communication et le jeu (CRC-JEU), UQAM  

---

## CV
- [Full Academic CV (PDF)](assets/cv/Yasmeen_CV_EN_Final.pdf)

---

## Contact
📧 abuhasirah.yasmeen@courrier.uqam.ca  
🔗 LinkedIn: https://www.linkedin.com/in/jasmeen-walid-abuhasirah-43645626  

---

_Last updated: January 2026_
